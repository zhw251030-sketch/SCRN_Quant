...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--row-label-style", choices=["metadata", "compact", "none"], default="compact")
    parser.add_argument("--panel-metric-style", choices=["snr", "none"], default="snr")
    parser.add_argument("--figure-title-style", choices=["full", "none"], default="none")
    parser.add_argument("--column-label-style", choices=["labels", "none"], default="labels")
    parser.add_argument("--colorbar-style", choices=["per_row", "none"], default="per_row")
    return parser
...
def main() -> None:
    ...
    selected_sets = select_requested_sets(rows, args.candidate_set, set_a_selection=str(args.set_a_selection))
        ...
        for set_id, selected_rows in selected_sets.items():
            generated.append(
                generate_candidate_set(
                    ...
                    figure_title_style=str(args.figure_title_style),
...
def select_requested_sets(
    ...
    candidate_set: str,
    *,
    set_a_selection: str = "condition_median",
) -> dict[str, list[dict[str, Any]]]:
    selected: dict[str, list[dict[str, Any]]] = {}
    if candidate_set in ("all", "set_a_three_degradation_levels"):
        if set_a_selection == "condition_median":
        ...
        elif set_a_selection == "fixed_patch_from_medium":
            selected["set_a_three_degradation_levels"] = select_fixed_patch_degradation_levels(rows)
        else:
            raise ValueError(f"Unknown set A selection mode: {set_a_selection}")
...
def select_fixed_patch_degradation_levels(rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    """Select light/medium/heavy rows from the same medium-representative patch."""
    medium = select_condition_representative(rows, snr_setting_db=1.0, missing_rate=0.18, row_label="medium")
    patch_file = str(medium["patch_file"])
    conditions = (
        ("light", 10.0, 0.02),
        ("medium", 1.0, 0.18),
        ("heavy", -2.0, 0.38),
    ...
    for row_label, snr_setting_db, missing_rate in conditions:
        matching = filter_condition(rows, snr_setting_db=snr_setting_db, missing_rate=missing_rate)
        condition_median = condition_medians(matching)
        row = next((item for item in matching if str(item["patch_file"]) == patch_file), None)
        if row is None:
            raise ValueError(f"Patch {patch_file} lacks SNR={snr_setting_db:g}, missing={missing_rate:g}")
        item = attach_selection(
            row,
            row_label=row_label,
            method="fixed_patch_from_medium_representative",
            score=representative_score(row, condition_median),
            medians=condition_median,
        )
        item["selection"]["fixed_patch_anchor"] = {
            "row_label": "medium",
            "patch_index": int(medium["patch_index"]),
            "patch_file": str(medium["patch_file"]),
            "source": str(medium.get("source", "unknown")),
            "snr_setting_db": 1.0,
            "missing_rate": 0.18,
        }
...
def generate_candidate_set(
    *,
    ...
    title = figure_title(set_id) if figure_title_style == "full" else ""
    save_visual_grid(
        png_path,
    ...
    manifest = build_manifest(
        ...
        row_label_style=row_label_style,
        panel_metric_style=panel_metric_style,
        figure_title_style=figure_title_style,
        column_label_style=column_label_style,
        colorbar_style=colorbar_style,
    )
...
def save_visual_grid(
    ...
    *,
    ...
    fig, axes = plt.subplots(len(display_rows), len(columns), figsize=(14.2, 7.2), constrained_layout=True)
    ...
    for row_index, row in enumerate(display_rows):
        ...
        for col_index, (key, label) in enumerate(columns):
            ...
            title_text = column_title_text(label, row_index=row_index, style=column_label_style)
            ...
            axis.axis("off")
        side_label = row_side_label(row, style=row_label_style)
        if side_label:
        ...
        if image is not None and should_draw_colorbar(colorbar_style):
            fig.colorbar(image, ax=axes_array[row_index, :].ravel().tolist(), shrink=0.76, fraction=0.018, pad=0.01)
    fig.savefig(path, dpi=300)
    plt.close(fig)


def column_title_text(label: str, *, row_index: int, style: str) -> str:
    if style == "none":
        return ""
    if style == "labels":
        return label if row_index == 0 else ""
    raise ValueError(f"Unknown column label style: {style}")


def should_draw_colorbar(style: str) -> bool:
    if style == "none":
        return False
    if style == "per_row":
        return True
    raise ValueError(f"Unknown colorbar style: {style}")


def row_side_label(row: Mapping[str, Any], *, style: str = "metadata") -> str:
    if style == "none":
        return ""
    if style == "compact":
        return compact_row_label(str(row["row_label"]))
    if style == "metadata":
        return (
            f"{row['row_label']}\n"
            f"{row['source']}\n"
    ...
    raise ValueError(f"Unknown row label style: {style}")


def compact_row_label(label: str) -> str:
    mapping = {"light": "Light", "medium": "Medium", "heavy": "Heavy"}
    if label in mapping:
        return mapping[label]
    match = re.fullmatch(r"sample_(\d+)", label)
    if match:
